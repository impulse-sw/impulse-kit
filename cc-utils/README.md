...
With `cc-utils`, you can:

- parse MsgPack at backend with `cc_utils::requests::MsgPackParser` extension trait (`salvo::Request`)
- send MsgPack from backend with `msgpack!(data)` macro
- parse MsgPack at client with `cc_utils::responses::MsgPackResponse` extension trait (`reqwest::Response`)
- send MsgPack from client with `cc_utils::requests::MsgPackRequest` extension trait (`reqwest::RequestBuilder`)
...
For MsgPack ser/de, `cc-utils` uses `rmp_serde::to_vec` and `rmp_serde::from_slice` methods.

## SIMD JSON support by `sonic-rs`

`cc-utils` provide:

- parse JSON at backend with `cc_utils::requests::SimdJsonParser` extension trait (`salvo::Request`)
...
`cc-utils` provide unified type `MResult<T>` for specifying response type while using `ServerError` for errors.

### `cc_utils::results::MResult`
...
This trait is implemented to all exposed by `cc-utils` response types above except `file_upload!` (internally it needs `req: &mut Request` to compare `ETag` inside headers to allow file caching).

## `cc_utils::results::CResult` and `cc_utils::errors::CliError`
