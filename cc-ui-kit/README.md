...
```rust
cc_ui_kit::setup_app(log::Level::Info, Box::new(move || { view! { <YourMainComponent /> }.into_any() }))
```
...
UI Kit exposes `cc_ui_kit::router::endpoint` to construct full URL of the backend, if this backend provided your frontend also.
...
If you need to go on any other page, use `cc_ui_kit::router::redirect`:
