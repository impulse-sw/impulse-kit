...
use smart_patcher::patch::{FilePath, FindRule, Patch, PatchFile, Replacer};
use smart_patcher::regex::RegexIR;
use std::fs;
      ...
      let patchfile = PatchFile {
        patches: vec![
          Patch {
            ...
            patch_find_graph: vec![FindRule::After(
              RegexIR::new("use cc_server_kit::prelude::\\*;").unwrap(),
            )],
            inplace: Some(ctx.new_types),
            ..Default::default()
          },
          Patch {
            files: vec![FilePath::Just(filename.to_owned())],
            patch_find_graph: vec![],
            replace: Some(Replacer::FromTo(ctx.old_router, ctx.new_router)),
            ..Default::default()
          },
          Patch {
            files: vec![FilePath::Just(filename.to_owned())],
            patch_find_graph: vec![FindRule::CursorAtEnd],
            inplace: Some(ctx.new_api_endpoints),
            ..Default::default()
          },
        ],
      };
