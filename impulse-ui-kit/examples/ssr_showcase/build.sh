...
mkdir -p ./impulse-ui-kit/examples/ssr_showcase/dist/pkg

printf '[7/7] Action `%bCompile TailwindCSS%b`...\n' "$BLUE" "$RESET"
printf 'Executing `%btailwindcss -i {tw-input} -o {tw-output} --minify%b`...\n' "$GREEN" "$RESET"
tailwindcss -i ./impulse-ui-kit/examples/ssr_showcase/input.css -o ./impulse-ui-kit/examples/ssr_showcase/dist/pkg/ssr_showcase.css --minify

mkdir -p artifacts
cp -rf target/x86_64-unknown-linux-gnu/release/ssr-showcase artifacts/ssr-showcase/ssr-showcase || true
cp -rf impulse-ui-kit/examples/ssr_showcase/server-example.yaml artifacts/ssr-showcase/server-example.yaml || true
cp -rf impulse-ui-kit/examples/ssr_showcase/dist artifacts/ssr-showcase/dist || true
