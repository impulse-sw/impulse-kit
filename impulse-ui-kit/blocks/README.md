...
```toml
[dependencies]
impulse-ui-kit-blocks = { git = "https://github.com/impulse-sw/impulse-kit.git", tag = "1.2.7" }
leptos = "0.8"
```

The crate mirrors the kit's feature flags — `csr` (default), `hydrate`, `ssr` —
and forwards them to `impulse-ui-kit` / `impulse-ui-kit-components`.
...
All charts live in `impulse_ui_kit_blocks::charts`. They share the same design:
...
`impulse_ui_kit_blocks::graph` is a small, interactive **node editor** (think a
