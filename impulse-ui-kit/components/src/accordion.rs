...
pub fn Accordion(
  ...
  let class_str = format!("{}{}", "w-full", class.map(|c| format!(" {}", c)).unwrap_or_default());
...
pub fn AccordionItem(
  ...
  let class_str = format!("{}{}", "border-b", class.map(|c| format!(" {}", c)).unwrap_or_default());

  view! { <div class=class_str>{children()}</div> }
}

#[component]
pub fn AccordionTrigger(#[prop(optional)] class: Option<String>, children: Children) -> impl IntoView {
  let accordion_ctx = use_context::<AccordionContext>().expect("AccordionTrigger must be used within an Accordion");
  let item_ctx1 = use_context::<AccordionItemContext>().expect("AccordionTrigger must be used within an AccordionItem");
  let item_ctx2 = use_context::<AccordionItemContext>().expect("AccordionTrigger must be used within an AccordionItem");

  let is_open = Memo::new(move |_| accordion_ctx.open_items.get().contains(&item_ctx1.value));
  ...
  let toggle_item = move |_| {
    ...
    match accordion_ctx.accordion_type {
      ...
      AccordionType::Multiple => {
        if current_items.contains(&item_value) {
          let filtered: Vec<String> = current_items.into_iter().filter(|item| item != &item_value).collect();
  ...
  view! {
    <h3 class="flex">
      <button
        ...
        type="button"
      >
        <span class="text-left">{children()}</span>
        <svg
...
#[component]
pub fn AccordionContent(#[prop(optional)] class: Option<String>, children: Children) -> impl IntoView {
  let accordion_ctx = use_context::<AccordionContext>().expect("AccordionContent must be used within an Accordion");
  let item_ctx = use_context::<AccordionItemContext>().expect("AccordionContent must be used within an AccordionItem");

  let is_open = Memo::new(move |_| accordion_ctx.open_items.get().contains(&item_ctx.value));
